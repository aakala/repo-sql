...
select  total.ts tablespace,
	...
	ELSE '['|| DECODE(free.mb,
		...
		'--------------------'))||']' 
         END as GRAPH
from
	(select tablespace_name ts, sum(maxbytes)/1024/1024 maxsize, sum(bytes)/1024/1024 mb from dba_data_files group by tablespace_name) total,
