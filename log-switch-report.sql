...
break on report
compute avg LABEL 'Average ' of total_count  on  report
compute avg LABEL 'Average ' of total_gb  on  report

select *  from (
Select x.*, round((x.total_count*y.log_size)/1024/1024/1024,2) "Total_GB" from
 (SELECT TO_CHAR(FIRST_TIME, 'DD/MM/YYYY') AS "DAY" 
...
, COUNT(FIRST_TIME) total_count
    FROM V$LOG_HISTORY
      WHERE extract(year FROM FIRST_TIME) >= extract(year FROM sysdate) -1
          GROUP BY TO_CHAR(FIRST_TIME, 'DD/MM/YYYY'))x,
		  (Select avg(Bytes) log_size from v$Log) y 
		  )
